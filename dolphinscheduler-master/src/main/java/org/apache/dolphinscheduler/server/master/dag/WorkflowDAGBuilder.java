...
public class WorkflowDAGBuilder {
    ...
    private final Map<String, DAGNode<TaskDefinition>> taskNameMap;

    private final Map<Long, DAGNode<TaskDefinition>> taskCodeMap;
    ...
    public WorkflowDAGBuilder addTaskNode(TaskDefinitionLog taskDefinition) {
        ...
        DAGNode<TaskDefinition> taskNode = DAGNode.<TaskDefinition>builder()
                .nodeName(taskName)
                .nodeValue(taskDefinition)
    ...
    public WorkflowDAGBuilder addTaskEdge(ProcessTaskRelationLog processTaskRelation) {
        ...
        if (taskCodeMap.containsKey(preTaskCode)) {
            DAGNode<TaskDefinition> fromTask = taskCodeMap.get(preTaskCode);
            if (taskCodeMap.containsKey(postTaskCode)) {
                DAGNode<TaskDefinition> toTask = taskCodeMap.get(postTaskCode);
